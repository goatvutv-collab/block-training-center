...
# --- TELA 2: CAMPO DE TREINAMENTO ---
    ...
    fases_json = {
        "DRIBLE": """{
            ...
            3: {startPos:{x:160, y:410}, gates:[{x1:220,x2:280,y:350},{x1:40,x2:100,y:280},{x1:220,x2:280,y:200},{x1:40,x2:100,y:120},{x1:130,x2:190,y:60}], 
                enemies:[{x:160, y:310, range:120, speed:1.8, dir:1},{x:160, y:230, range:150, speed:2.5, dir:-1},{x:160, y:140, range:100, speed:3.2, dir:1}]}
        }""",
    ...
    <script>
        ...
        let player = {{ x: 160, y: 410, speed: 3.2 }};
        ...
        // CONTROLE CENTRALIZADO ABAIXO DO CAMPO (Arcade Style)
        const joy = {{ x: 160, y: 530, baseRadius: 40, currX: 160, currY: 530, active: false }};
        ...
        function update() {{
            ...
            if (gameState === 'PLAYING') {{
                if (joy.active) {{
                    ...
                    if (d > 5) {{ // Pequeno deadzone para precisão
                        let vx = (dx/d)*player.speed, vy = (dy/d)*player.speed;
                        let nextX = player.x + vx, nextY = player.y + vy;
                        let margin = 18 * getScale(player.y);
                        
                        // LIMITES DO CAMPO (Paredes de 0 a 460px)
                        if(nextX > margin && nextX < 320 - margin) player.x = nextX;
                        if(nextY > margin && nextY < 450) player.y = nextY;
                        
                        ball.x += (player.x - ball.x) * 0.22;
                        ball.y += (player.y - 22*getScale(player.y) - ball.y) * 0.22;
                    }}
                }}
                ...
                if(phaseData.enemies) {{
                    phaseData.enemies.forEach(e => {{
                        ...
                        if (Math.hypot(player.x - e.x, player.y - e.y) < 20*getScale(e.y)) score -= 0.6;
                    }});
                }}
        ...
        function render() {{
            ...
            // Área do Controle (Abaixo do campo)
            ctx.fillStyle = '#000'; ctx.fillRect(0,460,320,140);
            ...
            drawList.forEach(obj => {{
                ...
                if (obj.type === 'gate') {{
                    ...
                    ctx.beginPath(); ctx.moveTo(obj.data.x1-8*s, obj.data.y); ctx.lineTo(obj.data.x1+8*s, obj.data.y); ctx.lineTo(obj.data.x1, obj.data.y-25*s); ctx.fill();
                    ctx.beginPath(); ctx.moveTo(obj.data.x2-8*s, obj.data.y); ctx.lineTo(obj.data.x2+8*s, obj.data.y); ctx.lineTo(obj.data.x2, obj.data.y-25*s); ctx.fill();
            ...
            // ARCADE JOYSTICK (Abaixo do campo)
        ...
        canvas.addEventListener('pointermove', e => {{ 
            ...
            const r=canvas.getBoundingClientRect(); 
            let dx=e.clientX-r.left-joy.x, dy=e.clientY-r.top-joy.y, d=Math.min(Math.hypot(dx,dy),40), a=Math.atan2(dy,dx); 
    ...
    if st.button("FINALIZAR E SALVAR", use_container_width=True):
        st.success(f"Evolução salva para {st.session_state.player_id}!")
        st.session_state.logged_in = False
        st.rerun()

st.sidebar.caption("GOAT TV ARCADE CT v4.0")
