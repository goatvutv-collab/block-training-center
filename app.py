...
if st.session_state.app_mode == 'LOBBY':
    ...
    with col2:
        ...
        st.info("💡 NOVO: Módulo 4 de Ritmo! Gabarite para bônus de +1.0!")
...
# --- TELA 2: CAMPO DE TREINAMENTO ---
    ...
    <script>
        ...
        function playWhistle(f, d=0.1) {{ const o = audioCtx.createOscillator(); const g = audioCtx.createGain(); o.type = 'triangle'; o.frequency.setValueAtTime(f, audioCtx.currentTime); g.gain.setValueAtTime(0.1, audioCtx.currentTime); o.connect(g); g.connect(audioCtx.destination); o.start(); o.stop(audioCtx.currentTime + d); }}
            ...
            if (gameState==='PLAYING') {{
                if (currentPhase < 4) {{
                    if (joy.active) {{
                        ...
                        if (d>3) {{ 
                            let vx=(dx/d)*player.speed, vy=(dy/d)*player.speed, m=20*(player.y/460*0.55+0.45);
                            if(player.x+vx>m && player.x+vx<320-m) player.x+=vx;
                    ...
                    pd.gates.forEach((g,i) => {{
                        ...
                        if(!fallenCones.includes(idA) && Math.hypot(player.x-g.x1,player.y-g.y)<12) {{ fallenCones.push(idA); score-=50; playWhistle(800); }}
                        if(!fallenCones.includes(idB) && Math.hypot(player.x-g.x2,player.y-g.y)<12) {{ fallenCones.push(idB); score-=50; playWhistle(800); }}
                    }});
                    ...
                    if(Math.floor(rhythmTimer*60) % (rhythmTimer<30?45:25)===0) rhythmNotes.push({{x:340, type:Math.floor(Math.random()*4), speed:rhythmTimer<30?4:7}});
                    rhythmNotes.forEach((n,i) => {{ n.x-=n.speed; if(n.x<-20) {{ rhythmNotes.splice(i,1); score-=15; missedAny=true; playWhistle(300); }} }});
        ...
        function render() {{
            ctx.fillStyle='#1e3d1a'; ctx.fillRect(0,0,320,460); ctx.fillStyle='#050505'; ctx.fillRect(0,460,320,140);
            if(currentPhase<4) {{
                ctx.strokeStyle="rgba(255,255,255,0.08)"; for(let i=-100; i<=420; i+=40) {{ ctx.beginPath(); ctx.moveTo(160,-80); ctx.lineTo(i*1.5-80,460); ctx.stroke(); }}
                let pd=phases[currentPhase];
                pd.gates.forEach((g,i) => {{
                    let s=(g.y/460*0.55+0.45); let idA=`p${{currentPhase}}g${{i}}a`;
                    ctx.fillStyle=fallenCones.includes(idA)?"rgba(255,100,0,0.3)":"#ff6600";
                    if(fallenCones.includes(idA)) {{ ctx.beginPath(); ctx.ellipse(g.x1,g.y,14*s,5*s,0,0,Math.PI*2); ctx.fill(); }}
                    else {{ ctx.beginPath(); ctx.moveTo(g.x1-8*s,g.y); ctx.lineTo(g.x1+8*s,g.y); ctx.lineTo(g.x1,g.y-22*s); ctx.fill(); }}
                ...
                ctx.fillRect(player.x-8,player.y-30,16,25); ctx.fillStyle="white"; ctx.beginPath(); ctx.arc(ball.x,ball.y,6,0,Math.PI*2); ctx.fill();
                ...
                rBtns.forEach((b,i) => {{
                    ctx.beginPath(); ctx.arc(b.x,b.y,40,0,Math.PI*2); ctx.strokeStyle="#ffd700"; ctx.lineWidth=2; ctx.stroke();
            ...
            if(gameState==='FINISHED') {{
                ...
                if(!missedAny && hits>0) {{ ctx.fillStyle="#fff"; ctx.font="bold 12px monospace"; ctx.fillText("GABARITO RÍTMICO: +1.0 EXTRA!", 160, 295); }}
            }}
        }}
        ...
        canvas.addEventListener('pointerdown', e => {{
            ...
            if(currentPhase<4) {{ if(Math.hypot(x-joy.x,y-joy.y)<65) joy.active=true; }}
                ...
                rBtns.forEach((b,i) => {{ if(Math.hypot(x-b.x,y-b.y)<45) {{
                    let h=false; rhythmNotes.forEach((n,ni) => {{ if(n.type===i && n.x>15 && n.x<85) {{
                        h=true; rhythmNotes.splice(ni,1); hits++;
                        score+=(rhythmTimer<30?20:50); player.juggleY=50; playWhistle(1000,0.05); if(navigator.vibrate) navigator.vibrate(30);
